...
class MarkdownTokenizer(HighlightTokenizer):
    ...
    def __init__(self):
        ...
        self.paragraphBreakRegex = QRegExp("^\\s*$")
        self.heading1SetextRegex = QRegExp("^===+\\s*$")
        self.heading2SetextRegex = QRegExp("^---+\\s*$")
        self.blockquoteRegex = QRegExp("^ {0,3}>.*$")
        self.githubCodeFenceStartRegex = QRegExp("^```+.*$")
        ...
        self.emphasisRegex.setMinimal(True)
        self.strongRegex = QRegExp("\\*\\*(?=\\S).*\\S\\*\\*(?!\\*)|__(?=\\S).*\\S__(?!_)")
        self.strongRegex.setMinimal(True)
        self.strikethroughRegex = QRegExp("~~[^\\s]+.*[^\\s]+~~")
        self.strikethroughRegex.setMinimal(True)
        self.verbatimRegex = QRegExp("`+")
        self.htmlTagRegex = QRegExp("<[^<>]+>")
        self.htmlTagRegex.setMinimal(True)
        self.htmlEntityRegex = QRegExp("&[a-zA-Z]+;|&#x?[0-9]+;")
        self.automaticLinkRegex = QRegExp("(<[a-zA-Z]+\\:.+>)|(<.+@.+>)")
        self.automaticLinkRegex.setMinimal(True)
        self.inlineLinkRegex = QRegExp("\\[.+\\]\\(.+\\)")
        self.inlineLinkRegex.setMinimal(True)
        self.referenceLinkRegex = QRegExp("\\[(.+)\\]")
        self.referenceLinkRegex.setMinimal(True)
        self.referenceDefinitionRegex = QRegExp("^\\s*\\[.+\\]:")
        self.imageRegex = QRegExp("!\\[.*\\]\\(.+\\)")
        self.imageRegex.setMinimal(True)
        self.htmlInlineCommentRegex = QRegExp("<!--.*-->")
        self.htmlInlineCommentRegex.setMinimal(True)
        self.mentionRegex = QRegExp("\\B@\\w+(\\-\\w+)*(/\\w+(\\-\\w+)*)?")
        self.pipeTableDividerRegex = QRegExp("^ {0,3}(\\|[ :]?)?-{3,}([ :]?\\|[ :]?-{3,}([ :]?\\|)?)+\\s*$")
